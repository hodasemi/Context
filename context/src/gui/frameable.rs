...
pub struct Frameable {
    ...
    resize_callbacks: RefCell<Vec<Box<dyn Fn() -> ()>>>,
...
impl Frameable {
    ...
    pub fn new(gui_handler: Arc<GuiHandler>) -> VerboseResult<Arc<Frameable>> {
        let frameable = Arc::new(Frameable {
            ...
            resize_callbacks: RefCell::new(Vec::new()),
    ...
    /// # Arguments
    ///
    /// * `callback` - is a `Box<Fn() -> ()>`
    pub fn add_callback(&self, callback: Box<dyn Fn() -> ()>) -> VerboseResult<()> {
        self.resize_callbacks.try_borrow_mut()?.push(callback);
        Ok(())
    }
    ...
    pub fn resize(&self) -> VerboseResult<()> {
        ...
        for callback in self.resize_callbacks.try_borrow_mut()?.iter() {
            (callback)();
        }
